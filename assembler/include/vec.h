...
{                                                                                   \
    logenter();                                                                     \
    die(pos > this->size || pos + len > this->size,                                 \
        "replace range. Out of range: start: %zu len: %zu, array len: 0..%zd",      \
        pos, len, (ssize_t)this->size);                                             \
    if (this->capacity < this->size + val_cnt) {                                    \
        this->capacity = this->capacity + this->size + val_cnt;                     \
        this->data = mem_realloc(this->data, this->capacity * sizeof(type));        \
    }                                                                               \
    ssize_t shift = (ssize_t)val_cnt - (ssize_t)len;                                \
    if (shift > 0) {                                                                \
        for (ssize_t i = (ssize_t)this->size - 1; i >= (ssize_t)(pos + len); i--) { \
            this->data[i + shift] = this->data[i];                                  \
        }                                                                           \
    } else if (shift < 0) {                                                         \
        for (ssize_t i = (ssize_t)(pos + len); i < (ssize_t)this->size; i++) {      \
            this->data[i + shift] = this->data[i];                                  \
        }                                                                           \
    }                                                                               \
    for (size_t i = 0; i < val_cnt; i++) {                                          \
